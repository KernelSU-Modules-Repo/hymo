...
int HymoFS::get_protocol_version() {
  int ret =
      syscall(SYS_reboot, HYMO_MAGIC1, HYMO_MAGIC2, HYMO_CMD_GET_VERSION, 0);
  if (ret < 0) {
...
bool HymoFS::clear_rules() {
  ...
  bool ret =
      syscall(SYS_reboot, HYMO_MAGIC1, HYMO_MAGIC2, HYMO_CMD_CLEAR_ALL, 0) == 0;
  if (!ret) {
...
bool HymoFS::add_rule(const std::string &src, const std::string &target,
  ...
  bool ret = syscall(SYS_reboot, HYMO_MAGIC1, HYMO_MAGIC2, HYMO_CMD_ADD_RULE,
                     &arg) == 0;
  if (!ret) {
...
bool HymoFS::add_merge_rule(const std::string &src, const std::string &target) {
  ...
  bool ret = syscall(SYS_reboot, HYMO_MAGIC1, HYMO_MAGIC2,
                     HYMO_CMD_ADD_MERGE_RULE, &arg) == 0;
  if (!ret) {
...
bool HymoFS::delete_rule(const std::string &src) {
  ...
  bool ret = syscall(SYS_reboot, HYMO_MAGIC1, HYMO_MAGIC2, HYMO_CMD_DEL_RULE,
                     &arg) == 0;
  if (!ret) {
...
bool HymoFS::set_mirror_path(const std::string &path) {
  ...
  bool ret = syscall(SYS_reboot, HYMO_MAGIC1, HYMO_MAGIC2,
                     HYMO_CMD_SET_MIRROR_PATH, &arg) == 0;
  if (!ret) {
...
bool HymoFS::hide_path(const std::string &path) {
  ...
  bool ret = syscall(SYS_reboot, HYMO_MAGIC1, HYMO_MAGIC2, HYMO_CMD_HIDE_RULE,
                     &arg) == 0;
  if (!ret) {
...
std::string HymoFS::get_active_rules() {
  ...
  int ret =
      syscall(SYS_reboot, HYMO_MAGIC1, HYMO_MAGIC2, HYMO_CMD_LIST_RULES, &arg);
  if (ret < 0) {
...
bool HymoFS::set_debug(bool enable) {
  ...
  bool ret = syscall(SYS_reboot, HYMO_MAGIC1, HYMO_MAGIC2, HYMO_CMD_SET_DEBUG,
                     &val) == 0;
  if (!ret) {
...
bool HymoFS::set_stealth(bool enable) {
  ...
  bool ret = syscall(SYS_reboot, HYMO_MAGIC1, HYMO_MAGIC2, HYMO_CMD_SET_STEALTH,
                     &val) == 0;
  if (!ret) {
...
bool HymoFS::hide_overlay_xattrs(const std::string &path) {
  ...
  bool ret = syscall(SYS_reboot, HYMO_MAGIC1, HYMO_MAGIC2,
                     HYMO_CMD_HIDE_OVERLAY_XATTRS, &arg) == 0;
  if (!ret) {
...
bool HymoFS::set_avc_log_spoofing(bool enabled) {
  ...
  bool ret = syscall(SYS_reboot, HYMO_MAGIC1, HYMO_MAGIC2,
                     HYMO_CMD_SET_AVC_LOG_SPOOFING, &arg) == 0;
  if (!ret) {
