...
fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::Build { release, sign_key: _ } => {
            ...
            // This ensures WebUI uses the exact paths defined in Rust.
...
fn generate_js_constants(root: &Path) -> Result<()> {
    ...
    for (rust_const, js_key, file_rel_path) in mappings {
        ...
        // JS side usually expects paths without trailing slash, but 'mnt/' is fine.
...
fn extract_str_constant(path: &Path, const_name: &str) -> Result<String> {
    ...
    anyhow::bail!("Constant not found")
}
