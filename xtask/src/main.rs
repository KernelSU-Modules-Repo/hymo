...
fn main() -> Result<()> {
    ...
    match cli.command {
        Commands::Build { release, sign_key } => {
            ...
            // 2. Build WebUI
            // This compiles frontend resources to module/webroot directory
            build_webui(&root)?;

            // 3. Build Zakosign (Host Tool)
            // Key: Build Host version of zakosign first for later signing
            let zakosign_bin = build_zakosign(&root)?;

            // 4. Build Core (Android)
            // Cross-compile meta-hybrid binary
            let core_bin = build_core(&root, release)?;

            // 5. Copy Module Files
            println!(":: Copying module files...");
            let module_src = root.join("module");
            // Ensure module.prop, customize.sh, etc. are at Zip root
            // Also includes the newly built webroot
            dir::copy(
                &module_src,
                &output_dir,
...
fn build_zakosign(root: &Path) -> Result<Option<PathBuf>> {
    ...
    if setup_script.exists() {
        ...
        // Ensure "host" argument is called to build Host tool
        println!(":: Running setupdep host...");
        let status_host = Command::new(&setup_script)
            ...
            .stderr(Stdio::inherit())
            .status()?;
        if !status_host.success() { anyhow::bail!("zakosign setupdep host failed"); }

        // Ensure dependencies are configured (no args)
        println!(":: Running setupdep dependencies...");
        let status_dep = Command::new(&setup_script)
            .current_dir(&zakosign_dir)
            .stdout(Stdio::inherit())
            .stderr(Stdio::inherit())
            .status()?;
        if !status_dep.success() { anyhow::bail!("zakosign setupdep failed"); }
    }
